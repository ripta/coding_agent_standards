...
# head/tail, and point the agent at cg_run instead. cg_run captures
...
if [[ "$TOOL" == "Bash" ]]; then
  ...
  if printf '%s' "$STRIPPED" | grep -qE '(\|&|&>|[0-9]*>{1,2}&?[0-9]*)'; then
    deny "Shell redirection is disabled by your safety hook. Use the cg_run MCP tool instead -- it captures stdout/stderr without needing 2>&1, so no redirection is necessary."
  fi

  if printf '%s' "$COMMAND" | grep -qE '\|[[:space:]]*(head|tail)([[:space:]]|$)'; then
    deny "Piping into head/tail is disabled by your safety hook. Use the cg_run MCP tool instead -- it truncates long output automatically, so you don't need to pipe to head/tail yourself."
  fi
fi
