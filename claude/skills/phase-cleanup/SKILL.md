...
description: |
  ...
  Updates the completed phases summary, proposal statuses, and 1ZP README index.
model: sonnet
...
1. `## Phase N: Title (1ZP-NNN)` heading
...
For each moved phase, check the corresponding 1ZP proposal file (`spec/1zp/NNN-*.md`):

1. Read its `**Status:**` line
2. If it says `scheduled`, change it to `implemented`
3. If it already says `implemented`, skip it

Then update the matching rows in `spec/1zp/README.md` index table to match.

### Step 8: Clean Up ideas.md

Read `spec/1zp/ideas.md`. For each idea section (`## Idea: ...`), check whether the idea has been captured in a numbered
1ZP proposal (regardless of that proposal's status). Cross-reference against the `spec/1zp/README.md` index.

An idea is "captured" if there is a 1ZP whose title or summary clearly covers the same feature or concern. When in
...
- Which ideas were removed from ideas.md and which 1ZP they corresponded to
